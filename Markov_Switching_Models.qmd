...
[**Brief Remarks on Notation**]{.underline}
...
[**Markov-Chains and Autoregressive Processes**]{.underline}
...
[**Markov-Switching Models (MSM)**]{.underline}
...
\displaystyle
&\sum_{t = m+1}^{T}\sum_{s_t = 1}^{N}\cdots\sum_{s_{t-m} = 1}^{N}\cfrac{(y_t - c_{s_t}^{(l+1)} -x_t'\phi^{(l+1)})x_t}{\sigma^_{(l+1)}^2}P_{\lambda_l}(S_t = s_t,...,S_{t-m} = s_{t-m}|\mathcal{Y}_T = \vec{y}_T) = 0
